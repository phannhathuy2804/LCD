...
//-----------------------------------------------------------------
void setAddrWindow(uint8_t write_data2[],uint8_t write_data,le_spi_DeviceHandleRef_t spiHandle)
{
	 write_data=0x2A;
...
void begin()
{
	...
	le_gpioPin7_SetPushPullOutput(LE_GPIOPIN7_ACTIVE_LOW,true);
	uint8_t write_data=ILI9341_SLPOUT;
	le_spi_WriteHD(spiHandle,&write_data,1);
	write_data=ILI9341_DISPON;
	le_spi_WriteHD(spiHandle,&write_data,1);
...
COMPONENT_INIT
{
	...
	begin();
	//Write Pixel
	setAddrWindow(write_data2,write_data,spiHandle);
	le_gpioPin7_SetPushPullOutput(LE_GPIOPIN7_ACTIVE_HIGH,true);
	write_data2[0]=0x07;
	write_data2[1]=0xE0;
	le_spi_WriteHD(spiHandle,write_data2,2);
